...
// use-case does not re-decide the race.
func (s *ResultService) SucceedExecution(ctx context.Context, executionID, actor string) error {
	...
	return s.publishExecutionEvent(ctx, event.ExecutionSucceeded, actor, run, succeeded.At)
}
...
func (s *ResultService) FailExecution(ctx context.Context, executionID, actor string) error {
	run, err := s.Executions.Get(ctx, executionID)
	if err != nil {
		return err
	}
	...
	return s.publishExecutionEvent(ctx, event.ExecutionFailed, actor, run, failed.At)
}

// publishExecutionEvent looks up the owning Task only for the event's
// ProjectID (Execution itself carries only a TaskID, per ADR-015: no
// cross-module reference beyond an id).
func (s *ResultService) publishExecutionEvent(ctx context.Context, eventType, actor string, run *execution.Execution, at time.Time) error {
