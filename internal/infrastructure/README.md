...
**Формат идентификатора репозитория и его нормализация ([BUGFIX-005](../../tasks/done/BUGFIX-005-github-repo-identifier-format.md)).** Платформа хранит идентификатор в каноническом виде `<host>/<owner>/<name>` (`github.com/nikibrdev/ai-studio-os` — именно это записывает `ProjectService.ConnectRepository`), а GitHub API требует в `/repos/{...}` голое `owner/name`. `normalizeRepo` снимает схему, хост и суффикс `.git` и **проверяет**, что осталось ровно два непустых сегмента: неразбираемый идентификатор даёт `ErrInvalidRepository` **до** HTTP-запроса. До этого исправления значение с хостом подставлялось в путь как есть, давая `/repos/github.com/owner/name/...` и невнятный `404`, похожий на отсутствующий репозиторий или недействительный токен. Дефект жил с EPIC-005 и был невидим, потому что тесты адаптера использовали только краткую форму, а все тесты уровнем выше — in-memory фейк провайдера (Open Question TASK-050: токена нет в каждой среде); первым его увидел живой прогон Orchestrator'а (TASK-082).
