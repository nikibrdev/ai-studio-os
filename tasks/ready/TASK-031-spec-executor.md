...
Полная, утверждённая спецификация `docs/specifications/domain/executor.md` по шаблону [Specification.md](../../.claude/templates/Specification.md) — техническое задание для будущей реализации `internal/domain/executor` (этап 2, не начинается без утверждения).
...
- `docs/specifications/domain/executor.md`: назначение; требования; чего модуль НЕ делает (не выполняет работу сам — это платформенный адаптер через `internal/platform.Executor`, домен-модуль только реестр/состояние); сценарии использования (регистрация, назначение на роль, отключение); инварианты; допустимые состояния (Registered → Active ⇄ Disabled → Retired); события; ограничения (соотношение с `internal/platform.Executor`, с понятием Agent — [ubiquitous-language.md](../../docs/domain/ubiquitous-language.md)); будущие расширения; Acceptance Criteria.
