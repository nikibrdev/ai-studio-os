...
| **Current Epic** | [EPIC-015 Проверенная работа агентов](docs/roadmap/EPIC-015-verified-agent-work.md) — открывает веху **v1.1**, TASK-106…109: закрыть единственный непроверенный участок v1.0 — содержательную работу агентов. Требует `ANTHROPIC_API_KEY` владельца проекта; всё, что можно сделать без ключа, идёт первым. **Публикация тега `v1.0.0` остаётся решением владельца** |
