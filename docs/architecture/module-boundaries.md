...
- **Разрешено:** публичные use-case-сервисы и проекции `internal/application` (терминология «модули Core» здесь предшествует [ADR-015](../adr/ADR-015-internal-layering.md); сегодняшний эквивалент — `internal/application`, не `internal/domain` напрямую, как и у `apps/api`); порт `application.EventJournal` — курсорное чтение журнала событий (EPIC-010/TASK-080: продуктивная шина доставляет только внутри своего процесса, ADR-002, поэтому подписка отдельному процессу недоступна); зависимости собираются через `internal/infrastructure/wiring.System`; контракт Executor; `pkg/`.
...
- **Реализованное узкое исключение** (EPIC-010/TASK-082, [apps/orchestrator/README.md](../../apps/orchestrator/README.md)): `main.go` как composition root импортирует конкретный адаптер `agents/claude-code`, чтобы его создать — сконструировать реализацию должен кто-то. Логика диспетчеризации (`Dispatcher`) при этом знает только `platform.Executor`: адаптер приходит к ней через поле-фабрику `NewExecutor func() (platform.Executor, error)` и нигде в ней не назван. Это то же разделение, что у `apps/api`, где `main.go` знает `wiring`/инфраструктуру, а хендлеры `httpapi` — нет; запрет выше о том, чтобы от провайдера не зависела логика, и он соблюдён.
...
| apps/dashboard | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ |
| apps/orchestrator | ✅ | ❌ | ✅ | ❌ | — | ✅ контракт | ❌ | ✅ сборка в `main.go` |
| pkg/ | ❌ | ❌ | — | ❌ | ❌ | ❌ | ❌ | ❌ |
