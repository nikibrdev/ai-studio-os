...
Канонический жизненный цикл задачи AI Studio OS: полный перечень состояний и правил переходов. Единственный источник истины для всех документов, workflow-определений и будущей реализации. Роли и организация процесса — в [workflow.md](workflow.md); события переходов — в [events.md](events.md). Два перехода из перечисленных ниже платформа сознательно не выполняет сама (Backlog → Ready и Testing → Done) — они контрольные точки человека, нормативно описанные в [workflow.md](workflow.md#контрольные-точки-человека); отдельными состояниями это не выражается.
