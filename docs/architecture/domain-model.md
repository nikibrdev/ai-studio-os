...
- **Ограничения:** переходы состояний — только разрешённые state machine; в работе — только задачи, прошедшие Definition of Ready; формат идентификатора — принят ([ADR-011](../adr/ADR-011-task-identifiers.md)): `TASK-NNN`, последовательный в рамках Project.
- **Владелец данных:** модуль `task`. Источник истины — принят ([ADR-004](../adr/ADR-004-task-storage.md)): PostgreSQL, `tasks/` — экспорт.
...
- **Ограничения:** именуется в прошедшем времени; схема события — часть публичного контракта модуля-источника; каталог событий — [events.md](events.md); механизм доставки — принят ([ADR-002](../adr/ADR-002-event-delivery.md)): внутрипроцессная шина с журналом в PostgreSQL.
...
- **Ограничения:** один PR — одна задача; слияние — только после одобренного Review; политика слияния — принята ([ADR-008](../adr/ADR-008-git-policies.md)): merge commit, слияние после Testing.
- **Владелец данных:** модуль `git`.
...
- **Ограничения:** процесс — по [review-process.md](../development/review-process.md); число обязательных ревью — принято ([ADR-008](../adr/ADR-008-git-policies.md)): один, агент-ревьюер допустим.
- **Владелец данных:** модуль `git`.
...
- **Ограничения:** модель аутентификации — принята ([ADR-012](../adr/ADR-012-identity-and-auth.md)): аутентификация отложена, доверенная однопользовательская установка.
...
- **Ограничения:** роль отделена от исполнителя; исполнители ролей PM и QA — приняты ([ADR-007](../adr/ADR-007-pm-qa-executors.md)): обе роли агенты, человек подтверждает на контрольных точках.
...
- **Ограничения:** зависит от модели аутентификации — принята ([ADR-012](../adr/ADR-012-identity-and-auth.md)), аутентификация отложена.
- **Владелец данных:** модуль `identity`.
