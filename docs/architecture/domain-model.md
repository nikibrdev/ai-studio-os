...
classDiagram
    ...
    Execution "0..*" --> "1" Executor : выполняется
    Execution "1" o-- "0..*" Artifact : производит
...
- **Назначение:** осязаемый результат выполненной работы — коммит, Pull Request, документ, ADR, лог тестового прогона, диаграмма и т.п. Первичная сущность результата работы платформы — не Result, не Output, не Response ([ADR-005](../adr/ADR-005-executor-contract.md), решение архитектора 2026-07-20): система производит именно артефакты, задачи — лишь способ организовать работу к их производству.
...
- **Связи:** производится Execution напрямую (без промежуточной сущности); опосредованно связан с Task через Execution.
- **Ограничения:** артефакт всегда имеет тип и источник; удаление запрещено (только архивирование).
- **Владелец данных:** модуль `execution` (точное размещение — отдельный ли модуль `artifact` или владение остаётся за `execution` — открытый вопрос [ADR-005](../adr/ADR-005-executor-contract.md), не решается этим документом).

#### Execution (Исполнение)

- **Назначение:** один запуск Executor'а для выполнения задачи или шага workflow.
- **Ответственность:** фиксирует, кто, что, когда и с каким контекстом выполнял; отслеживает ход выполнения; производит Artifact и несёт статус (`ExecutionStatus`) напрямую — без промежуточной сущности Result ([ADR-005](../adr/ADR-005-executor-contract.md)).
- **Жизненный цикл:** Queued → Running → Succeeded | Failed | Aborted.
- **Связи:** порождается Task (в рамках WorkflowStep); выполняется Executor; вызывает Tool; производит 0..* Artifact.
