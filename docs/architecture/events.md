...
- **Источник:** модуль `task` (по команде роли Project Manager; **не переход** — задача остаётся в Backlog).
- **Получатели:** журнал, проекция Dashboard, Orchestrator.
- **Данные:** идентификатор задачи, уточнённый scope и/или критерии приёмки — **только фактически изменённые поля**. Отсутствие ключа означает «не уточнялось», а не «очищено»: домен запрещает очистку (`SetScope("")` — ошибка), поэтому пустое значение не является допустимым уточнением, и частичное уточнение не должно стирать соседнее поле.
- **Последствия:** человек видит подготовленное описание задачи **до** приёма Definition of Ready — это контрольная точка, и уточнение её не проходит ([workflow.md](workflow.md#контрольные-точки-человека)). Допустимо только в Backlog: дальше scope перестаёт быть предложением.
