...
Веб-интерфейс для людей: наблюдение за процессом. Работает только через API. Первая версия ([EPIC-009](../roadmap/EPIC-009-dashboard.md), v0.8) — read-only (список проектов, список и детали задач); формы управления (создание проекта, запуск задачи и т.п.) сознательно отложены до реальной потребности.
