...
**`GET /healthz`** — отвечает `200`, если процесс запущен и готов обслуживать запросы. Единственная операция вне ресурсной модели: не относится ни к одному ресурсу и не вызывает use-case. Готовность **не** включает проверку внешних зависимостей (PostgreSQL, GitHub, Qdrant) — их недоступность проявится ошибкой конкретной операции, а не общим отказом живости.
