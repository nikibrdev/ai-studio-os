...
**Доставка секретов при старте** (TASK-106): значения передаются через `docker run --env-file` — временный файл, доступный только текущему пользователю и удаляемый сразу после старта, — а не флагами `-e ИМЯ=значение`, оставлявшими секрет в argv процесса `docker` (argv на хосте читает любой локальный пользователь). Остаточная видимость через `docker inspect` принята осознанно: она требует доступа к Docker-сокету, эквивалентного root на хосте. Подробнее — [agents/claude-code](../../agents/claude-code/README.md).
