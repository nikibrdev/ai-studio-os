...
- [~] **Частично** — `apps/orchestrator` на `TaskPlanned` автоматически ведёт задачу Ready → In Progress через реальный `agents/claude-code`, без вызова `apps/api` человеком: подтверждено вживую (создание ветки в GitHub, запуск песочницы, клон, опрос статуса, уничтожение контейнера). Участок «сбор коммитов как Artifact → Pull Request → Review» **вживую не пройден**: без `ANTHROPIC_API_KEY` агент не производит коммитов, поэтому успешного исхода не наступает. Этот участок покрыт юнит-тестами по всем путям (TASK-083, 9 тестов), но тестами, а не живым прогоном — отмечено частично сознательно, см. Open Questions.
- [x] Сценарий подтверждён вживую на реальной инфраструктуре (PostgreSQL + Docker + настоящий приватный репозиторий и действительный токен) — TASK-085, три полных прогона; ограничение по отсутствию `ANTHROPIC_API_KEY` явно задокументировано в отчёте и в Open Questions ниже.
...
- [x] PROJECT_MANIFEST/PROJECT_HEALTH/ROADMAP/CHANGELOG синхронизированы при закрытии — TASK-085.
...
| TASK-085 | Живая проверка на реальной инфраструктуре, закрытие эпика | done |
...
Платформа впервые **сама** доводит запланированную задачу до работающего исполнителя: `TaskPlanned` → выбор исполнителя → ветка в GitHub → `StartTask` → запуск изолированной песочницы с клоном → слежение до терминального состояния → уничтожение песочницы. Без участия человека и без вызовов `apps/api` руками — подтверждено тремя живыми прогонами на настоящей инфраструктуре (TASK-085).

**Чего эпик не достиг:** участок «коммиты агента → Pull Request → Review» вживую не пройден — без `ANTHROPIC_API_KEY` агент не производит коммитов, поэтому успешного исхода не наступает. Логика этого участка покрыта юнит-тестами по всем путям, но не живым прогоном; критерий отмечен частично, вопрос вынесен в Open Questions.

**Цена честной проверки: четыре дефекта в уже закрытых эпиках.** Два найдены при разработке ([BUGFIX-004](../../tasks/done/BUGFIX-004-projection-rebuild-loses-envelope-data.md)/[005](../../tasks/done/BUGFIX-005-github-repo-identifier-format.md)), два — сквозной живой проверкой ([BUGFIX-006](../../tasks/done/BUGFIX-006-container-clone-url-doubled-host.md)/[007](../../tasks/done/BUGFIX-007-artifacts-reports-inherited-history.md)), и один из них (BUGFIX-006) означал, что **ни одно** реальное исполнение не могло начаться с самого EPIC-006. Все четыре — в швах между слоями, все невидимы для существовавших тестов, у всех одна общая причина: каждая сторона шва тестировалась своими данными, а не теми, которые реально передаёт соседний слой.

## Open Questions

1. **`ANTHROPIC_API_KEY` для проверки содержательной работы агента.** Наследуется из EPIC-006 (Open Question TASK-056), теперь блокирует последний непроверенный участок этого эпика. Код к прогону готов — нужен только секрет; предлагается закрыть отдельной короткой задачей при первой возможности, а не переоткрытием эпика.
2. **Выбор исполнителя слишком наивен.** Живая проверка вскрыла, что при нескольких Active-исполнителях с ролью Developer Orchestrator выбирает первого по id — им оказалась запись, оставшаяся от интеграционных тестов (`exec-store-1`), — но фактически запускает **свой** бэкенд через `NewExecutor`. В событиях фиксируется один исполнитель, а работает другой. На чистой установке запись одна, поэтому расхождение не проявляется. Требуется правило выбора (предпочитать собственную запись бутстрапа либо связать `NewExecutor` с выбранной записью) — за пределами scope этого эпика, но до появления второго исполнителя (эпик «Роли PM/QA») решить нужно.
3. **Устойчивое хранение курсора** — после перехода на `seq` (TASK-080) это одно целое число в таблице; открыто до появления реальной потребности.
