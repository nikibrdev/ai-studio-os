...
- **`TaskProjection.Get` — только одна запись по ID**, списковых операций (все задачи проекта и т.п.) нет — самый заметный практический предел этой версии API для будущего Dashboard; расширение проекций — решение по реальной потребности, не здесь.
