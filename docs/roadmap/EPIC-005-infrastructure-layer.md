...
| --- | --- | --- |
| TASK-046 | Каркас `internal/infrastructure`: конфигурация подключения, `pgxpool`, раннер миграций, Docker Compose, README | ready |
...
- **Docker недоступен на машине разработчика в моменте** (проверено при открытии эпика: `docker ps` — daemon не запущен) — интеграционные тесты (TASK-051) будут написаны и включены в CI (сервис-контейнер поднимается GitHub Actions независимо от локальной машины), но локальный прогон против реальной БД потребует запуска Docker Desktop человеком; риск снимается тем, что тесты пропускаются (`t.Skip`) при отсутствии `TEST_DATABASE_URL`, а не падают.
