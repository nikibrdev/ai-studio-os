...
- **GitHub-адаптер не проверен против реального API** — TASK-050 покрыт unit-тестами на HTTP-уровне (httptest, 89.6%), но ручная проверка на тестовом репозитории не выполнена (нет тестового репозитория и PAT в этой сессии) и вне scope без отдельного решения человека о хранении секрета в CI; риск остаётся открытым (см. Open Questions TASK-050).
